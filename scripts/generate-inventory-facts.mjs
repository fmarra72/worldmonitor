...
export function buildInventoryFacts(stats = loadStatsForInventoryFacts()) {
  ...
  // mcpTools is legitimately 0 in this fork: api/mcp/ is intentionally
  // excluded from deployment (kept out of Vercel's Serverless Functions
  // count — see .vercelignore), so its tool catalog is empty by design,
  // not a broken measurement.
  const ZERO_ALLOWED_CAPABILITIES = new Set(['mcpTools']);
  for (const [name, value] of Object.entries(capabilities)) {
    if (name === 'localeCodes') continue;
    if (!Number.isInteger(value) || (value <= 0 && !ZERO_ALLOWED_CAPABILITIES.has(name))) {
