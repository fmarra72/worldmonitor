...
function computeStats({ sourceAttribution: suppliedAttribution } = {}) {
  ...
  // The full api/mcp/ tool-server implementation is intentionally excluded
  // from this deployment (kept out of Vercel's Serverless Functions count —
  // see .vercelignore). Its UI-app inventory is a docs/mcp-apps.mdx
  // consistency check (docs:check only, never hit by this build), so a
  // missing tree here degrades to an empty inventory instead of failing
  // computeStats() for every caller that just wants unrelated stats
  // (source-attribution provider counts, tier1Countries, etc).
  let mcpApps;
  try {
    mcpApps = parseMcpAppsInventory();
  } catch (error) {
    if (error?.code !== 'ENOENT') throw error;
    mcpApps = { specVersion: '', mimeType: '', apps: [], uiResources: [], linkedTools: [], toolLinks: [] };
  }
