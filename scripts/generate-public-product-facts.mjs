...
// api/mcp/registry/index.ts (and everything it transitively imports — 47
// files) is intentionally excluded from this deployment: kept out of
// Vercel's Serverless Functions count (see .vercelignore). It's still the
// authoritative tool catalog when present (local dev, or a future
// redeploy that re-includes it), so load it dynamically and degrade to an
// empty catalog rather than failing this whole script when it's absent.
let TOOL_REGISTRY = [];
let toolAccess = () => ({});
try {
  const mcpRegistry = await import('../api/mcp/registry/index.ts');
  TOOL_REGISTRY = mcpRegistry.TOOL_REGISTRY;
  toolAccess = mcpRegistry.toolAccess;
} catch (error) {
...
function buildDepthProofStats(hero) {
  ...
  // mcpTools is legitimately 0 in this fork: the api/mcp/ tool-server
  // implementation is intentionally excluded from deployment (kept out of
  // Vercel's Serverless Functions count — see .vercelignore), and this
  // fork never builds the /pro marketing bundle (build:pro) that displays
  // depthProofStats, so a zero here documents an intentional product
  // decision rather than a silently broken measurement.
  const ZERO_ALLOWED_KEYS = new Set(['mcpTools']);
  for (const [key, value] of Object.entries(candidate)) {
    if (!Number.isInteger(value) || (value <= 0 && !ZERO_ALLOWED_KEYS.has(key))) {
